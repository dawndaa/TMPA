...
class Shifter_seg(nn.Module):
    def __init__(self, 
        ...
        self.embed_dim = text_embeds.shape[-1]     
        self.per_label = per_label   
        self.num_classes = num_classes    
        ...
        if per_label:    
            shift_init = torch.zeros((text_embeds.shape[0], text_embeds.shape[1]), dtype=self.dtype)     
            scale_init = torch.ones((text_embeds.shape[0], text_embeds.shape[1]), dtype=self.dtype)    
        else:
            shift_init = torch.zeros(self.embed_dim, dtype=self.dtype)
            scale_init = torch.ones(self.embed_dim, dtype=self.dtype)
...
class Shifter_visual(nn.Module):
    def __init__(self, 
            dtype=None, 
            do_shift=True, 
            do_scale=False,
        ...
        if per_label:    
            shift_init = torch.zeros((36,1), dtype=self.dtype)     
            scale_init = torch.ones((self.num_classes), dtype=self.dtype)     
        else:
            shift_init = torch.zeros(self.embed_dim, dtype=self.dtype)
            scale_init = torch.ones(self.embed_dim, dtype=self.dtype)
    ...
    def forward(self, img_embed, feat, query_idx, test=False):
        print(self.shift)
