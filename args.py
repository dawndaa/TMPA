...
def parse_args():
    ...
    parser.add_argument(
        ...
        action='store_true',
        help=(
            'Disable reliability only in RSAP: keep consensus class assignment, '
            'sample K fixed evenly spaced candidate positions, average uniformly '
            'and fuse with alpha only. SDR still uses the original reliability map. '
            'Requires --module_rsap_v1.'
        ),
    )
    parser.add_argument(
        '--no_sdr_reliability',
        action='store_true',
        help=(
            'Disable reliability only in SDR: use uniform symmetric-KL source '
            'consistency with the same lamb_sdr. RSAP is unchanged. Requires --loss_sdr.'
        ),
    )
    parser.add_argument(
        '--no_reliability',
        action='store_true',
        help=(
            'Disable reliability in both RSAP and SDR. Equivalent to combining '
            '--no_rsap_reliability and --no_sdr_reliability; requires Full RSAP + SDR.'
        ),
    )
    parser.add_argument(
        '--rsap_gamma',
    ...
    if args.no_reliability:
        if not (args.module_rsap_v1 and args.loss_sdr):
            raise ValueError('--no_reliability requires Full: --module_rsap_v1 --loss_sdr.')
        args.no_rsap_reliability = True
        args.no_sdr_reliability = True

    if args.no_rsap_reliability and not args.module_rsap_v1:
        raise ValueError('--no_rsap_reliability requires --module_rsap_v1.')
    if args.no_sdr_reliability and not args.loss_sdr:
        raise ValueError('--no_sdr_reliability requires --loss_sdr.')
